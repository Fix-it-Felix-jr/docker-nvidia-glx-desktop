...
RUN apt-get update && apt-get install --no-install-recommends -y \
    ...
    cd /tmp && curl -fsSL -o nvidia_cuda_nvrtc_linux_x86_64.tar.xz "https://developer.download.nvidia.com/compute/cuda/redist/cuda_nvrtc/linux-x86_64/cuda_nvrtc-linux-x86_64-12.3.107-archive.tar.xz" && mkdir nvrtc && tar -xf nvidia_cuda_nvrtc_linux_x86_64.tar.xz -C nvrtc --strip-components 1 && cd nvrtc/lib && chmod 755 libnvrtc* && find . -maxdepth 1 -type f -name "*libnvrtc.so.*" -exec sh -c 'ln -snf $(basename {}) libnvrtc.so' \; && mv -f libnvrtc* /opt/gstreamer/lib/x86_64-linux-gnu/ && cd /tmp && rm -rf /tmp/* && \
